...
const App = () =>{
  ...
  const requestFetchWeather =  () => {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${location}&units=imperial&appid=a8f11dfb3faef3021f175e8ee7ad7e41`)
  ...
  return (
    <div className='bg-gray-800 h-screen grid place-items-center'> 
      ...
      <p>Weather</p>
      <div>{fetchWeather.weather ? <p>{fetchWeather.weather[0].main}</p> : null}
      </div>
      <p>Feels Like</p>
      <div>{fetchWeather.weather ? <p>{fetchWeather.main.feels_like}°F</p> : null}
      </div>
      <p>Wind</p>
      <div>{fetchWeather.wind ? <p>{fetchWeather.wind.speed}MPH</p> : null}
      </div>
      </div>
    </div>
  );
